...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.use_res_connect = self.stride == 1 and inp == oup  #判断是否使用残差连接，需要满足两个条件，特征大小不变即步长为1和输入通道数和输出通道数是否相同

        if expand_ratio == 1:#如果不进行通道数扩张
            ...
            self.conv = nn.Sequential(
                ...
                #   利用1x1卷积进行通道数的上升
                #-----------------------------------#
                #输入通道数，输出通道数（等于 inp × expand_ratio），kernel_size=1即1×1，stride=1即步长为1，padding = 0，bias=False
                ...
                #--------------------------------------------#
                #Depthwise 卷积
                #输入通道，输出通道，3×3卷积核，步长，padding=1，groups=hidden_dim每个输入通道用自己的卷积核卷积，bias=False
                nn.Conv2d(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False),
                BatchNorm2d(hidden_dim),
                nn.ReLU6(inplace=True),
                #-----------------------------------#
